...
**Assuming the interim SCC correctly reflects the total social cost of one metric ton of CO2, the marginal cost per kwH of electricity is $0.02**
...
ggplot() +
  ...
  geom_segment(aes(x = 0, y = 2, xend = 819713.75, yend = 2, color = "MEC", lty = "MEC")) +
  geom_line(MPC, mapping = aes(x = demand, y = price + 2, color = "MSC", lty = "MSC")) +
...
**The benefit to consumers is \$52987.22. The benefit to producers is \$26835.97. The environmental cost under the status quo is $10734.39.**
...
l_pred_new <- tibble(price = price + 2,
                 demand = demand(price, model_demand_l))

l_pred_new[l_pred_new == 0] <- NA

h_pred_new <- tibble(price = price + 2,
                 demand = demand(price, model_demand_h))

#create a new Dagg
demand_agg_new <- function(p){
  q <- demand(p + 2 , model_demand_l) + demand(p + 2, model_demand_h)
  return(q)
}
...
ggplot() +
  ...
  geom_line(MPC, mapping = aes(x = demand, y = price, color = "MPC", lty = "MPC")) +
  geom_segment(aes(x = 0, y = 2, xend = 819713.75, yend = 2, color = "MEC", lty = "MEC")) +
  geom_line(agg_data_new, mapping = aes(x = Qagg_new, y = price, color = "New Aggregate", lty = "New Aggregate")) +
  scale_color_manual(name = "Legend", values = cal_palette("kelp1")) +
  scale_linetype_manual(name = "Legend", values = c("dotdash", "dashed", "dashed", "solid", "solid", "solid")) +
  labs(x = "Quantity (kWh)",
       y = "Price (cents)",
       color = "Demand Curve") + 
  theme_minimal()
...
fm_eq <- function(p)((demand(p + 2 , model_demand_l) + demand(p + 2, model_demand_h))-(p/MPC_slope))
peq <- uniroot(fm_eq, c(0,600000))$root
qeq <- demand_agg_new(peq)
```

**Optimal tax is $0.02. The amount of electricity that is produced is 499660.54 kWh, and the price per kWh is $0.09.**
...
**The welfare of high income consumers is \$44,874.79, as they do not bear the environmental costs.**

(d) Overall welfare of “low” income consumers

```{r}
CS_low_new <- CS_low/100 - env_cost
```

**The welfare of low income consumer is negative due to having to bear the environmental cost. It is -$10653.27**
...
**The welfare of power suppliers is $23258.02**
...
**The total environmental damage is $9993.21**
...
**Total tax revenue generated is $9993.21**
