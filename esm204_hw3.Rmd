...
for(i in 1:length(MEC_vec)){
  ...
  CShigh_new <- CS(peq_new, model_demand_h)/100
  
  results <- cbind(i, peq_new, qeq_new, encost_new, CSlow_new, CShigh_new) %>% unlist(recursive = FALSE)
  
  values <- bind_rows(values, results)
}
```
```{r}
fm_eq_i <- function(p){
    (demand(p + MEC_75 , model_demand_l) + demand(p + MEC_75,model_demand_h))-(p/MPC_slope)
  }

peq_75 <- uniroot(fm_eq, c(0,600000))$root
