...
demand_halved <- function(p, model){
  q <- ((0.5*p) - model$coefficients[[1]])/model$coefficients[[2]]
  q <- ifelse(q<0,0,q)
  return(q)
}
...
demand_agg_6 <- function(p){
  q <- demand_halved(p, model_demand_l) + demand(p, model_demand_h)
  return(q)
}
...
peq_6 <- uniroot(freemarket_6, c(0,600000))$root
qeq_6 <- demand_agg_6(peq_6)
```

**Total electricity consumption is 500216.43 kWh**
