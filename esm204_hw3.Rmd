...
l_pred_new <- tibble(price = price + 1.97,
                 demand = demand(price, model_demand_l))

l_pred_new[l_pred_new == 0] <- NA

h_pred_new <- tibble(price = price + 1.97,
                 demand = demand(price, model_demand_h))
...
fm_eq <- function(p)((demand(p + 1.97 , model_demand_l) + demand(p + 1.97, model_demand_h))-(p/MPC_slope))
peq <- uniroot(fm_eq, c(0,600000))$root
qeq <- demand_agg_new(peq)
```

**Optimal tax is 1.97 cents. The amount of electricity that is produced is 500216.43 kWh, and the price per kWh is 9.32 cents.**
...
CSh_new <- CS(peq, model_demand_h)/100
```

**The welfare of high income consumers is \$44,874.79 ($47744.28???), as they do not bear the environmental costs.**

(d) Overall welfare of “low” income consumers

```{r}
env_cost_new <- (1.97 * qeq)/100
CS_low_new <- CS(peq, model_demand_l)/100 - env_cost_new/100
```

**The welfare of low income consumer is negative due to having to bear the environmental cost. It is -$2460.94 (-$9854.26??)**
...
for(i in 1.96:MEC_vec){
  
  fm_eq <- function(p)((demand(p + i , model_demand_l) + demand(p + i,model_demand_h))-(p/MPC_slope))
  
  peq_new <- uniroot(fm_eq, c(0,600000))$root
  
  qeq_new <- demand_agg_new(peq)
  
  encost_new <- i * qeq
  
  CSlow_new <- CS(peq_new, model_demand_l)/100 - encost_new/100
  
  CShigh_new <- CS(peq, model_demand_h)/100
  
  results <- c(i, peq_new, qeq_new, encost_new, CSlow_new, CShigh_new) %>% unlist()
  
  values <- map_dfr(results, bind_rows)
}
```
