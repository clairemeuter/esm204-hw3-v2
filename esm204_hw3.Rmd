...
ggplot() +
  ...
  scale_linetype_manual(name = "Legend", values = c("dotdash", "solid", "dashed", "dashed", "solid", "solid", "solid", "solid")) +
  labs(x = "Quantity (kWh)",
       y = "Price (cents)",
       color = "Demand Curve") + 
  theme_minimal()
...
for(i in MEC_vec){
  ...
  qeq_new <- demand_agg_fl(peq_new)
  
  encost_new <- i * qeq_new
  
  CSlow_new <- CS_new(peq_new, model_demand_l)/100 - encost_new/100
  
  CShigh_new <- CS_new(peq_new, model_demand_h)/100
