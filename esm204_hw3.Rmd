...
ggplot() +
  ...
  scale_color_manual(name = "Legend", values = cal_palette("kelp1")) +
  scale_linetype_manual(name = "Legend", values = c("dotdash", "solid", "dashed", "dashed", "solid", "solid")) +
  labs(x = "Quantity (kWh)",
       y = "Price (cents)",
       color = "Demand Curve") + 
  theme_minimal()
...
demand_agg <- function(q){
  p <- demand(q, model_demand_l) + demand(q, model_demand_h)
  return(p)
}
...
diff <- demand_agg(peq_6) - demand_agg_6(peq_6)
```
